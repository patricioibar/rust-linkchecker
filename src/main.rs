...
use tokio::io::{
    AsyncBufRead, AsyncBufReadExt, AsyncWrite, AsyncWriteExt, BufReader, BufWriter, stdout,
};
use tokio::sync::mpsc::{self, Receiver, Sender};

const NUMBER_OF_REQUEST_WORKERS: usize = 32;

/// Simple program to accept an input file and optional output file.
#[derive(Parser)]
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.output {
        Some(out) => match File::create(&out).await {
            Ok(file) => process_file(input_file, BufWriter::new(file)).await,
            Err(err) => {
                eprintln!("Error creating output file '{}': {}", out, err);
                std::process::exit(1);
            }
        },
        None => process_file(input_file, BufWriter::new(stdout())).await,
    }
}
